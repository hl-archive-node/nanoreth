use crate::node::primitives::{HlTxType, TransactionSigned};
...
impl ReceiptBuilder for RethReceiptBuilder {
    ...
    fn build_receipt<E: Evm>(&self, ctx: ReceiptBuilderCtx<'_, HlTxType, E>) -> Self::Receipt {
        let ReceiptBuilderCtx { tx_type, result, cumulative_gas_used, .. } = ctx;
        let HlTxType::Default(tx_type) = tx_type;
        Receipt {
            tx_type,
